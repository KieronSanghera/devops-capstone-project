...
def read_account(account_id):
    """
    Reads an Account
    This endpoint will read an Account from the account_id that is requested
    """
    app.logger.info("Request to read an Account with the id: %s", account_id)
    account = Account.find(account_id)
    if not account:
        abort(status.HTTP_404_NOT_FOUND, f"Account with the id {account_id} could not be found.")
...
def delete_accounts(account_id):
    ...
    """
    app.logger.info("Request to delete account with the id: %s", account_id)
    account = Account.find(account_id)
    if not account:
        abort(status.HTTP_404_NOT_FOUND, f"Account with the id {account_id} could not be found.")
    account.delete()
